...
### Fixed
- **False Color Caching**: Implemented a caching mechanism for false color data to improve performance.
  - When switching between false color types (e.g., ARRI to Sony), the processed data is now cached.
  - Subsequent switches to an already-processed type will load from the cache, eliminating the need for reprocessing and reducing wait times.
- **Loading Spinner Logic**: Corrected the behavior of the loading spinner.
  - The spinner now only appears when new false color data is being processed by the server.
  - It no longer gets stuck on the screen when switching back to a cached false color mode.
